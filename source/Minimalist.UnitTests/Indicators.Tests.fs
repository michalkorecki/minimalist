...
[<Test>]
let ``Directional movement equals to distance between today H and yesterday H when it is the largest`` () =
...
[<Test>]
let ``Directional movement equals to distance between yesterday's L and today's L when it is the largest``() =
